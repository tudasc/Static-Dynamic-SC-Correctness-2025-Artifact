...
/*
{
    ...
    "RACE_PAIR": ["nvshmem_double_atomic_set@37","nvshmem_long_atomic_set@42"],
...
__global__ void nvshmem_kernel(double *remote, double *localbuf) {
    // Initialize memory
    *remote = 0;
    *localbuf = 1;
    double x = 0;
    ...
    if (my_pe == 0) {
        // CONFLICT
        nvshmem_double_atomic_set(remote, 2.0, 1);
    }

    if (my_pe == 2) {
        // CONFLICT
        nvshmem_long_atomic_set((long*)remote, 3, 1);
    }
