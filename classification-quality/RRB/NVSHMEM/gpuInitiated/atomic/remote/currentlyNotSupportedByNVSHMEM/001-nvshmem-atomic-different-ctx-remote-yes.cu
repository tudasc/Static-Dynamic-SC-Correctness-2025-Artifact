...
/*
{
    ...
    "RACE_PAIR": ["nvshmem_ctx_int_atomic_add@43","nvshmem_ctx_int_atomic_add@49"],
...
__global__ void nvshmem_kernel(int *remote, int *localbuf) {
    ...
    *localbuf = 1;
    ...
    // Create contexts
    nvshmem_ctx_t ctx_world;
    nvshmem_ctx_t ctx_shared;
    nvshmem_team_create_ctx(SHMEM_TEAM_WORLD, 0, &ctx_world);
    nvshmem_team_create_ctx(SHMEM_TEAM_SHARED, 0, &ctx_shared);
    ...
    if (my_pe == 0) {
        // CONFLICT
        nvshmem_ctx_int_atomic_add(ctx_world, remote_d, 1, 1);
    }

    if (my_pe == 2) {
        int target_pe = nvshmem_team_translate_pe(SHMEM_TEAM_WORLD, 1, nvshmem_TEAM_SHARED);
        // CONFLICT
        nvshmem_ctx_int_atomic_add(ctx_shared, remote_d, 1, target_pe);
    }
