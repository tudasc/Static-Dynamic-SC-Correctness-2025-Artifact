...
/*
{
    ...
    "RACE_PAIR": ["nvshmem_int_put_signal@37","nvshmem_int_put_signal@42"],
    "NPROCS": 3,
...
__global__ void nvshmem_kernel(int *remote, int *localbuf) {
    ...
    *localbuf = 1;
    ...
    if (my_pe == 0) {
        // CONFLICT
        nvshmem_int_put_signal(remote, localbuf, 1, &ps_sig_addr0, 1, NVSHMEM_SIGNAL_SET, 1);
    }

    if (my_pe == 2) {
        // CONFLICT
        nvshmem_int_put_signal(remote, localbuf, 1, &ps_sig_addr2, 1, NVSHMEM_SIGNAL_SET, 1);
    }
