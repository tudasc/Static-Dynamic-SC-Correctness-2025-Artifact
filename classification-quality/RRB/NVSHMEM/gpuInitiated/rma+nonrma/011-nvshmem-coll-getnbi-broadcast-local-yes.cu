...
__global__ void nvshmem_kernel(int *remote, int *remote_alt, int *localbuf) {
    // Initialize memory
    *remote = 0;
    *localbuf = 1;
    int x = 0;
    ...
    // CONFLICT
    nvshmem_int_broadcast(NVSHMEM_TEAM_WORLD, remote_alt, localbuf, 1, 0);
...
int main(int argc, char **argv) {
    int remote, localbuf, remote_alt;
    ...
    int *remote_alt_d = (int *)nvshmem_malloc(sizeof(int));
    ...
    void *args[] = {remote_d, remote_alt_d, localbuf_d};  // Kernel arguments
    dim3 blocks(1);
    dim3 threads(1);
    ...
    cudaMemcpy(&remote_alt, remote_alt_d, sizeof(int), cudaMemcpyDeviceToHost);
