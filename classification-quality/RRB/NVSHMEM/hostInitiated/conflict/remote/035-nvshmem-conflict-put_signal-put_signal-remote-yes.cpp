...
/*
{
    ...
    "RACE_PAIR": ["nvshmem_int_put_signal@54","nvshmem_int_put_signal@59"],
    "NPROCS": 3,
    "DESCRIPTION": "Two conflicting operations putsignal and putsignal executed concurrently which leads to a race."
}
*/
// RACE LABELS END
...
int main(int argc, char **argv) {
    int remote, localbuf;
    static uint64_t ps_sig_addr0 = 0;
    ...
    if (my_pe == 0) {
        // CONFLICT
        nvshmem_int_put_signal(remote_d, localbuf_d, 1, &ps_sig_addr0, 1, NVSHMEM_SIGNAL_SET, 1);
    }

    if (my_pe == 2) {
        // CONFLICT
        nvshmem_int_put_signal(remote_d, localbuf_d, 1, &ps_sig_addr2, 1, NVSHMEM_SIGNAL_SET, 1);
    }
