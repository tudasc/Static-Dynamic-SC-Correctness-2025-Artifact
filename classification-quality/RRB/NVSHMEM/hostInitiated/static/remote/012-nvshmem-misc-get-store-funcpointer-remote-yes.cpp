...
/*
{
    ...
    "RACE_PAIR": ["nvshmem_int_get@23","cudaMemcpy@28"],
    "NPROCS": 2,
...
__attribute__((noinline)) void rank0(int* rem_ptr_d, int* lbuf_ptr_d, int* x) { 
    // CONFLICT
...
__attribute__((noinline)) void rank1(int* rem_ptr, int* lbuf_ptr, int* x) {
    // CONFLICT
    cudaMemcpy(rem_ptr, x, sizeof(int), cudaMemcpyHostToDevice);
}
...
int main(int argc, char **argv) {
    ...
    void (*rankfunc)(int* rem_ptr, int* lbuf_ptr, int* x);
    ...
    (*rankfunc)(rem_ptr_d, lbuf_ptr_d, &remote);
