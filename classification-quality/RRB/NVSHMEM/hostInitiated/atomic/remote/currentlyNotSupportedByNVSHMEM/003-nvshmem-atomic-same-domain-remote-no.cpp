...
int main(int argc, char** argv) {
    ...
    // Create contexts
    nvshmem_ctx_t ctx1, ctx2;
    nvshmem_ctx_create(0, &ctx1);
    nvshmem_ctx_create(0, &ctx2);

    // Synchronize across all PEs
    nvshmem_barrier_all();

    if (my_pe == 0) {
        nvshmem_ctx_int_atomic_add(ctx1, remote_d, 1, 1);
    }

    if (my_pe == 2) {
        nvshmem_ctx_int_atomic_add(ctx2, remote_d, 1, 1);
    }
