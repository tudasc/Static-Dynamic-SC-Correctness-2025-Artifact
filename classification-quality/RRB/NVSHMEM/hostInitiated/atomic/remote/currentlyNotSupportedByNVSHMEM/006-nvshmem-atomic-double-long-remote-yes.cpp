...
/*
{
    ...
    "RACE_PAIR": ["nvshmem_double_atomic_set@52","nvshmem_long_atomic_add@57"],
...
int main(int argc, char** argv) {
    ...
    double *remote_d = (double *)nvshmem_malloc(sizeof(double));
    double *localbuf_d = (double *)nvshmem_malloc(sizeof(double));

    // Initialize memory
    cudaMemset(remote_d, 0, sizeof(double));
    cudaMemset(localbuf_d, 1, sizeof(double));
    ...
    if (my_pe == 0) {
        // CONFLICT
        nvshmem_double_atomic_set(remote_d, 2.0, 1);
    }

    if (my_pe == 2) {
        // CONFLICT
        nvshmem_long_atomic_set((long*)remote_d, 3, 1);
    }
    ...
    cudaMemcpy(&remote, remote_d, sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(&localbuf, localbuf_d, sizeof(double), cudaMemcpyDeviceToHost);
