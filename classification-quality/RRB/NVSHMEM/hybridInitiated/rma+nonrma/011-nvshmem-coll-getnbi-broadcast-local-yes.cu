...
int main(int argc, char **argv) {
    ...
    cudaMemset(remote_alt_d, 0, sizeof(int));
    ...
    nvshmem_kernel<<<blocks, threads, shared_data_size>>>(remote_alt_d, localbuf_d);

    if (my_pe == 0) {
        // CONFLICT
        nvshmem_int_broadcast(NVSHMEM_TEAM_WORLD, remote_alt_d, localbuf_d, 1, 0);
    }
