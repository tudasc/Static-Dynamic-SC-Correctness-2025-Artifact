...
__global__ void nvshmem_kernel(int *remote, int *localbuf, int i) {
    // CONFLICT
    localbuf[i-1] = 42;
}
...
int main(int argc, char **argv) {
    ...
    if (my_pe == 0) {
        for (int i = 1; i < ARR_SIZE; i++) {
            // CONFLICT
            nvshmem_int_put_nbi(remote_d+i, localbuf_d+i, 1, 1);
            nvshmem_kernel<<<blocks, threads, shared_data_size>>>(remote_d, localbuf_d, i);
        }
    }
